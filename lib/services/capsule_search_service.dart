...
class CapsuleSearchService {
  ...
  static const double _similarityThreshold = 0.05; // Lower threshold for our approach
  static const int _maxResults = 5;
  
  final Map<String, List<Map<String, dynamic>>> _embeddingsCache = {};
  bool _isInitialized = false;

  static final CapsuleSearchService _instance = CapsuleSearchService._internal();
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _isInitialized = true;
      print('CapsuleSearchService initialized with ${_embeddingsCache.length} capsules');
    } catch (e) {
  ...
  Future<void> _loadEmbeddingFile(String filePath) async {
    try {
      // Load JSON embeddings file
      final jsonContent = await rootBundle.loadString(filePath);
  ...
  Future<CapsuleSearchResult> search(String query, {int maxResults = _maxResults}) async {
    await initialize();
    ...
    for (final entry in _embeddingsCache.entries) {
      ...
      for (final item in embeddings) {
        ...
        final semanticSimilarity = _calculateCosineSimilarity(
          queryEmbedding, 
          List<double>.from(item['embedding'])
        );

        // Calculate keyword-based similarity
        final keywordSimilarity = _calculateKeywordSimilarity(query, item['content']);
        
        // Combine both scores with weights
        final combinedScore = (semanticSimilarity * 0.4) + (keywordSimilarity * 0.6);
  ...
  bool _isStopWord(String word) {
    const stopWords = {
      'the', 'and', 'for', 'are', 'but', 'not', 'you', 'can', 'her', 'was', 'one',
      'our', 'had', 'by', 'what', 'were', 'they', 'we', 'when', 'your',
      'said', 'each', 'which', 'she', 'how', 'other', 'than', 'now', 'very', 'my', 'be',
      'has', 'he', 'in', 'will', 'on', 'it', 'of', 'an', 'as', 'is', 'his', 'have',
      'that', 'to', 'a', 'with', 'at', 'this', 'or', 'from', 'if', 'all'
    };
